...
describe('/games', () => {
    let agent, app, mongoose, gamesService;

    before((done) => {
        require('../../src/config/mongoose').then((_mongoose) => {
            mongoose = _mongoose;
            app = require('../../src/app')(mongoose);
            gamesService = require('../../src/services/games')(_mongoose);
            done();
        }).catch(done);
    });
